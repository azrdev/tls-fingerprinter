...
public class FingerprintFuzzer extends AFingerprintAnalyzer {
	 ...
	 */
	public FingerprintFuzzer(String testcase, ETLSImplementation impl, EStates state,
	...
	public void analyze(ArrayList<MessageContainer> traceList) {
        ...
        try {
			behaviour.insertFingerprint(parameters, traceList, this.state.name(), this.testcase, this.impl.name());
		} catch (Exception e) {
			e.printStackTrace();
		}
