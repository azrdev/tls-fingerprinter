...
public class TcpHeader extends Header {
    ...
    public class TcpSession extends Signature {
        ...
        public TcpSession() {
            this.sourcePort = getSourcePort();
            this.destinationPort = getDestinationPort();

            IpHeader ipHeader = (IpHeader) previousHeader;
            this.sourceAddress = ipHeader.getSourceAddress();
    ...
    public boolean isContinuedBy(PcapPacket packet) {
        if (packet.hasHeader(Id)) {
            TcpHeader tcpHeader = packet.getHeader(Id);
            return getNextSequenceNumber() == tcpHeader.getSequenceNumber();
        }
