...
import de.rub.nds.research.ssl.stack.tests.response.ServerHelloHandler;
...
public class SSLHandshakeWorkflow extends AWorkflow {
	...
	public void start() {
		...
		addToList(new Trace(States.CLIENT_HELLO, trace.getCurrentRecord(), trace.getOldRecord(), false));
		...
		/* Not all states might be run through in the server response.
		 * It has to be assured that after server response ServerHelloDone
		 * state is reached.
		 */
		...
		if (keyExAlg == EKeyExchangeAlgorithm.RSA){
            ...
            pms = new PreMasterSecret(privateValue, keyParams.getDhPublic(), primeModulus);
		}
		cke.encode(true);
		...
		hashBuilder.updateHash(msg,5,msg.length-5);
		//add trace to ArrayList
		addToList(new Trace(States.CLIENT_KEY_EXCHANGE, trace.getCurrentRecord(), trace.getOldRecord(), false));
		...
		utils.sendMessage(out, msg);
		encrypted=true;
		addToList(new Trace(States.CHANGE_CIPHER_SPEC, trace.getCurrentRecord(), trace.getOldRecord(), false));
		...
		try {
			masterSec = new MasterSecret(param.getClientRandom(), param.getServerRandom(), pre_master_secret);
		...
		Finished finished = msgBuilder.createFinished(protocolVersion, EConnectionEnd.CLIENT, handshakeHashes, masterSec);
		...
		TLSCiphertext rec = new TLSCiphertext(protocolVersion, EContentType.HANDSHAKE);
		...
		addToList(new Trace(States.FINISHED, trace.getCurrentRecord(), trace.getOldRecord(), false));
	...
	private void getResponses(HandshakeHashBuilder hashBuilder, Trace trace){
		...
		try {
			while (in.available()!=0){
				...
				//hash current record
				hashBuilder.updateHash(responseBytes,5,responseBytes.length-5);
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
