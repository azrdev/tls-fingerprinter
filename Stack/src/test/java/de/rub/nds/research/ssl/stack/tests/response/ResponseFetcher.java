...
public class ResponseFetcher implements Observer {
    ...
    public void update(Observable o, Object arg) {
        ...
        if (states != null) {
            if (states == EStates.DATA_SEND) {
                ...
                EStates currentState = workflow.getTraceList().get(
                        traceListSize - 1).getState();
                switch (currentState) {
                    case CLIENT_HELLO:
                        try {
                            fetchNextHeader();
                        ...
                        while (workflow.getCurrentState() != EStates.SERVER_HELLO_DONE.
                                getID()) {
                            this.extractAndHashMessage();
                            if (workflow.getCurrentState() != EStates.SERVER_HELLO_DONE.
                                    getID()
                                    && workflow.getCurrentState() != EStates.ALERT.
                                    getID()) {
                                try {
                                    fetchNextHeader();
                                } catch (IOException e) {
                            ...
                            } else if (workflow.getCurrentState() == EStates.ALERT.
                                    getID()) {
                                logger.info(
                                        "### Connection reset due to FATAL_ALERT.");
                                workflow.closeSocket();
                                return;
                            }
                    ...
                    case CLIENT_KEY_EXCHANGE:
                        try {
                            if (dis.available() > 0) {
                                fetchNextHeader();
                                this.extractAndHashMessage();
                                if (workflow.getCurrentState() == EStates.ALERT.
                                        getID()) {
                                    logger.info(
                                            "### Connection reset due to FATAL_ALERT.");
                                    workflow.closeSocket();
                    ...
                    case CLIENT_FINISHED:
                        ...
                        while (workflow.getCurrentState() != EStates.SERVER_FINISHED.
                                getID()) {
                            this.extractAndHashMessage();
                            if (workflow.getCurrentState() != EStates.SERVER_FINISHED.
                                    getID()
                                    && workflow.getCurrentState() != EStates.ALERT.
                                    getID()) {
                                try {
    ...
    private void setRecordHeader(byte[] header) {
        ...
        if (header != null) {
            System.arraycopy(header, 0, tmp, 0,
