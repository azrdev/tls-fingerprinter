...
public class PseudoRandomFunction {
	...
	public PseudoRandomFunction(int length){
		PseudoRandomFunction.length=length;
		try {
	...
	public byte [] generatePseudoRandomValue(byte [] secret, String label, byte [] seed) throws InvalidKeyException{
		...
		System.arraycopy(secret, secret_length-part_length, secret2, 0, part_length);
		float rounds = 0.0f;
	 ...
	 */
	private byte[] p_sha1(final byte[] secret, final byte[] seed) throws InvalidKeyException{
	 ...
	 * @throws InvalidKeyException
	 */
	private byte[] p_hash(byte [] secret, byte [] seed, final int rounds) throws InvalidKeyException{
		byte [] a0 = seed;
		byte [] a1 = null;
		byte [] a2 = null;
		byte [] output = null;
		...
		switch (this.hashAlgorithm) {
			case MD5:output = new byte[MD5_BLOCK_LENGTH* rounds];
			a1 = p_md5(secret, a0);
			currentHash = a1;
			break;
			case SHA1:output = new byte[SHA1_BLOCK_LENGTH* rounds];
			a1 = p_sha1(secret, a0);
			currentHash = a1;
			break;
			default: break;
